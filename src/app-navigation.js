...
const APP_NAV_ICONS = {
	// Icons from https://heroicons.com/ with some custom ones for the font generator and converter.
	'home.svg': `
...
function getNavigationIcon(href) {
	...
	if (href.includes('email')) {
		return APP_NAV_ICONS['email.svg'];
	}

	if (href.includes('name')) {
		return APP_NAV_ICONS['name.svg'];
	}

	if (href.includes('string')) {
		return APP_NAV_ICONS['string.svg'];
	}
...
function buildBackButton() {
	return `
		<a href="/apps" class="inline-flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-lg text-xs font-medium text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition duration-150 transform">&lt; BACK</a>
	`;
}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	document.querySelectorAll('[data-app-back-button]').forEach((container) => {
		container.innerHTML = buildBackButton();
	});
});